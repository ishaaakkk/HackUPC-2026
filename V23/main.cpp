...
std::vector<std::string> buildLocationQueries(const json& vision) {
    ...
    if (queries.size() > 10) queries.resize(10);
    return queries;
}
...
json inferLocations(const json& originalVision, const Options& options) {
    ...
    while (static_cast<int>(verified.size()) > options.maxCandidates) verified.erase(verified.end() - 1);
...
json buildSimpleLocationsOutput(const json& fullResult) {
    json simple;
    simple["source_input"] = fullResult.value("source_input", fullResult.value("source_url", ""));
    simple["source_type"] = fullResult.value("source_type", "url");
    simple["media_type"] = fullResult.value("media_type", "");
    simple["exact_location_found"] = false;
    simple["confidence_level"] = "unknown";
    simple["possible_locations"] = json::array();

    if (!fullResult.contains("location_inference")) {
        return simple;
    }

    const json& inference = fullResult["location_inference"];
    simple["exact_location_found"] = inference.value("exact_location_found", false);
    simple["confidence_level"] = inference.value("confidence_level", "unknown");

    for (const auto& c : inference.value("candidate_locations", json::array())) {
        json item;
        item["name"] = c.value("name", "");
        item["formatted_address"] = c.value("formatted_address", "");

        json coordinates;
        if (c.contains("latitude") && !c["latitude"].is_null()) {
            coordinates["latitude"] = c["latitude"];
        } else {
            coordinates["latitude"] = nullptr;
        }

        if (c.contains("longitude") && !c["longitude"].is_null()) {
            coordinates["longitude"] = c["longitude"];
        } else {
            coordinates["longitude"] = nullptr;
        }

        item["coordinates"] = coordinates;
        simple["possible_locations"].push_back(item);
    }
...
json processInput(const std::string& input, const Options& options) {
    ...
    out["note"] = "final_confidence es un score heurístico, no una probabilidad matemática exacta.";
    return out;
}
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        json simpleResult = buildSimpleLocationsOutput(result);
        std::ofstream simpleOut("output_locations_simple.json");
        simpleOut << simpleResult.dump(2);
        simpleOut.close();

        std::cout << "Análisis completado. Resultados creados:\n";
        std::cout << "  - output_location.json\n";
        std::cout << "  - output_locations_simple.json\n";
        curl_global_cleanup();
        return 0;
    } catch (const std::exception& e) {
