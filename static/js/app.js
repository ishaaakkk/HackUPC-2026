...
(function () {
  ...
  btnConfirm.addEventListener("click", async () => {
    ...
    // Build destinations payload. Do NOT pass only monument names to Skyscanner.
    // Each destination keeps the detected place coordinates, plus flight_search_city
    // generated by the backend, e.g. "Sagrada Família" -> "Barcelona".
    const destinationsPayload = state.confirmedLocations.map((l) => ({
      city: l.city || l.name,
      name: l.name || l.city,
      country: l.country || "",
      latitude: l.latitude,
      ...
      maps_url: l.maps_url || "",
      flight_search_city: l.flight_search_city || l.flight_search_label || l.city || l.name,
    }));
  ...
  async function renderFlightDashboard() {
    ...
    for (const dest of destNames) {
      ...
      const destLat = Number(info.lat);
      const destLon = Number(info.lon);
      if (!Number.isFinite(destLat) || !Number.isFinite(destLon) || (destLat === 0 && destLon === 0)) {
        console.warn("Destination skipped because it has no usable coordinates", dest, info);
        continue;
      }
      ...
      const locData = state.confirmedLocations.find((l) => {
        const names = [l.city, l.name, l.flight_search_city, l.flight_search_label].filter(Boolean).map((x) => String(x).toLowerCase());
        return names.includes(String(dest).toLowerCase()) || names.includes(String(info.destination_name || "").toLowerCase());
      });

      const marker = L.marker([destLat, destLon], { icon: destIcon }).addTo(
        mapLayers
      );
      ...
      // Dashed line
      L.polyline([[originLat, originLon], [destLat, destLon]], {
        color: "#667eea",
        weight: 2,
        opacity: 0.5,
        dashArray: "8, 12",
  ...
  async function openDestPanel(destName, flightInfo, locData) {
    ...
    // Fill data
    panelCity.textContent = flightInfo.destination_name || destName;
    panelCountry.textContent = locData?.country || flightInfo.country || "";
    ...
    // Fetch image
    const imgUrl = await getWikiImage(flightInfo.destination_name || destName);
